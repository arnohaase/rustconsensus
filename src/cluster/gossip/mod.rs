...
pub async fn run_gossip(config: Arc<ClusterConfig>, messaging: Arc<Messaging>, cluster_state: Arc<RwLock<ClusterState>>) -> anyhow::Result<()> {
...
async fn do_gossip(gossip: &Gossip, messaging: &Messaging) { //TODO move somewhere else
...
async fn reply(sender: NodeAddr, message: GossipMessage, messaging: &Messaging) {
    let mut buf = BytesMut::new();
    message.ser(&mut buf);
...
async fn on_gossip_message(msg: GossipMessage, sender: NodeAddr, gossip: &mut Gossip, messaging: &Messaging) { //TODO move to 'gossip_messages.rs'
    use GossipMessage::*;
